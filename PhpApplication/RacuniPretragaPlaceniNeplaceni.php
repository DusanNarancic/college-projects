						...
						if ($db_selected)
										   {
											...
											$SQL = "select distinct jePlacen from `".$bazapodataka."`.`Racun` ORDER BY jePlacen ASC";
											//$SQL = "SELECT `".$bazapodataka."`.`Usluga`.IdUsluge,`".$bazapodataka."`.`Usluga`.IdKorisnika,`".$bazapodataka."`.`Racun`.jePlacen FROM `".$bazapodataka."`.`Racun` INNER JOIN `".$bazapodataka."`.`Usluga` ON `".$bazapodataka."`.`Racun`.IdUsluge=`".$bazapodataka."`.`Usluga`.IdUsluge WHERE `".$bazapodataka."`.`Usluga`.IdKorisnika='".$idkorisnika."'";
											$resultplaceni = mysql_query($SQL);
											$num_rowsplaceni = mysql_num_rows($resultplaceni);
...
<?php include 'delovi/desnoracunipretraga.php';?>
